    ...
    <div class="col-xl-12">
        <div class="card">
            <div class="card-body">
                ...
                <h5 class="font-size-16 me-3 mb-0">Uploads</h5>
                <p class="text-muted">Silahkan unggah berkas-berkas yang dibutuhkan (Maks. 1 Mb)</p>
                <div class="row mt-4">
                    <div class="card-body p-3">
                        <form action="{{ route('file-manager.upload') }}" class="dropzone" id="exampleDropzone"
                            method="post">
                            @csrf
...
@section('scripts')
    ...
    <script>
        Dropzone.options.exampleDropzone = {
            url: '{{ route('file-manager.upload') }}', // Sesuaikan dengan URL backend-mu
            maxFiles: 5, // Maksimal jumlah file yang bisa diunggah
            acceptedFiles: 'image/*', // Hanya menerima file gambar
            addRemoveLinks: true, // Tambahkan tombol hapus file
            dictRemoveFile: "Hapus", // Ubah teks tombol hapus
            maxFilesize: 1, // Ukuran maksimal file dalam MB
            paramName: "files",
            uploadMultiple: true,
            init: function() {
                this.on("success", function(file, response) {
                    console.log("File berhasil diunggah:", response);
                    window.location.href = "{{ route('file-manager') }}";
                });
